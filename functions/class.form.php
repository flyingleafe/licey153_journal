...
class Form
{
	...
	public function __construct( $handler = '' ) {
		...
		if( $handler ) {
			...
			$data = $wpdb->get_row("SELECT * FROM `".$table_schedule."` WHERE ".$way."='".$handler."';");
			$this->id = 	$data->id;
			$this->form = 	$data->form;
	...
	public function add_new( $form ) {
		...
		try {
			...
			$exist_form = $wpdb->get_results("SELECT * FROM `".$table_schedule."` WHERE form='".$form."';");
			...
			//запрос к БД
			$wpdb->insert($table_schedule, $arr, array('%s','%s','%s','%s','%s','%s','%s','%s') );
			$wpdb->insert($table_subjects, array('form' => $form), array('%s') );	
	...
	public function delete($way = 'id') {
		...
		$wpdb->query("DELETE FROM `".$table_schedule."` WHERE `".$way."`=".$handler.";");
		$wpdb->query("DELETE FROM `".$table_schedule."` WHERE `form`=".$this->form.";");
	...
	public function get_students() {
		...
		$stud_ids = $wpdb->get_col("SELECT * FROM `" . $table_students . "` WHERE form='" . $this->form . "' ORDER BY student_name;");
	...
	public function updateDaySchedule($day, $schedule)
	{
		...
		$wpdb->update( $table_schedule, array( $day => json_encode($schedule) ), array( 'form' => $this->form ), array('%s'), array('%s') );
	}
